...
const icon_style = icons
  .map(({ name }) => create_styles(`.file.file_type_${name}`, name))
  .join("\n");
...
const folder_icon_style = [
  ...
  folder_icons
    .map(({ name, folder_name }) => {
      let css = folder_name.map((val) => {
        return [
          create_styles(
            `#file-browser > ul > li.tile[type="directory"][name="${val}"] > .icon.folder`,
            name,
          ),
...
class material {
  async init() {
    ...
    this.folder_icon_style = <style textContent={folder_icon_style}></style>;
